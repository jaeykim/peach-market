...
export default function DealWorkflow({
  ...
  const steps: Step[] = [
    { key: "match", label: "매칭 완료", desc: "양측 가격 합의", status: "DONE" },
    { key: "verify", label: "본인 인증", desc: "PASS / NICE 실명 확인", status: "PENDING", action: "준비 중" },
    { key: "registry", label: "등기부 검증", desc: "권리관계·근저당 자동 확인", status: "READY", action: "매물 페이지에서 조회" },
    {
      key: "contract",
      label: "표준 계약서 생성",
    ...
    },
    {
      ...
      desc: bothSigned
        ...
        ? "내 서명 완료, 상대방 대기"
        : "캔버스에 직접 서명",
      status: bothSigned ? "DONE" : mySigDone ? "READY" : "READY",
    },
    { key: "escrow", label: "에스크로 입금", desc: "은행 가상계좌로 잔금 안전 보관", status: "PENDING", action: "준비 중" },
    { key: "notary", label: "법무사 매칭 + 등기 이전", desc: "검증된 법무사 풀에서 자동 매칭", status: "PENDING", action: "준비 중" },
    { key: "tax", label: "세금 신고 안내", desc: "취득세 / 양도세 자동 계산", status: "PENDING", action: "준비 중" },
    { key: "complete", label: "거래 완료", desc: "등기부 정리 후 종결", status: "PENDING" },
  ];
      ...
      <ol className="space-y-2">
        {steps.map((s, i) => (
          ...
          >
            ...
            {s.key === "contract" && s.status === "READY" && isSeller && (
              <button
            ...
            {s.key === "contract" && s.status === "READY" && !isSeller && (
              <span className="text-[10px] text-neutral-400">매도인 권한</span>
            )}
